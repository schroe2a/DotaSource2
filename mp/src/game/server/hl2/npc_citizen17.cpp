...
void CNPC_Citizen::StartTask( const Task_t *pTask )
{
	...
	case TASK_CIT_HEAL_TOSS:
		...
		if ( IsMedic() )
		{
			if ( GetTarget() && GetTarget()->IsPlayer() && GetTarget()->GetMaxHealth() == GetTarget()->m_iHealth )
			{
...
bool CNPC_Citizen::ShouldHealTarget( CBaseEntity *pTarget, bool bActiveUse )
{
			...
			)
	 	{
			if ( pTarget->m_iHealth > 0 )
			{
	 			if ( bActiveUse )
				{
					...
					if ( healAmt > pTarget->GetMaxHealth() - pTarget->m_iHealth )
						healAmt = pTarget->GetMaxHealth() - pTarget->m_iHealth;
					if ( healAmt < sk_citizen_heal_player_min_forced.GetFloat() )
						return false;

	 				return ( pTarget->GetMaxHealth() > pTarget->m_iHealth );
				}
	...
	if ( IsAmmoResupplier() && bTargetIsPlayer )
	{
		...
		{
			//always at least try to give your dude ammo
			return true;
		}
	}
	return false;
...
void CNPC_Citizen::Heal()
{
	...
	if ( IsAmmoResupplier() )
	{
		...
		if ( pTarget->IsPlayer() )
		{
			CHL2MP_Player * pPlayer = ToHL2MPPlayer( pTarget );
			Dota_Resupply::ReSupplyPlayer( pPlayer );
